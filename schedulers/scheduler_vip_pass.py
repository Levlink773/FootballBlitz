...
"""
    ...
    def __init__(self, user: UserBot):
        ...
        self._user_id: int = user.user_id
        
        self._end_time: datetime = user.vip_pass_expiration_date
    ...
    async def _send_message(self):
        try:
            ...
            logger.error(f"Failed to send message to {self.user.user_name}\nError: {E}")
            
    async def start_taimer(self):
        self.scheduler.add_job(
            func = self._send_message,
            trigger = self.trigger_end_date_vip_pass,
            name = f"end_time_vip_pass_{self.user.user_id}",
