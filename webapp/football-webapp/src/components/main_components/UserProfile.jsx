...
const AgeIcon = ({className}) => (
    ...
    >
        {/* ... (код SVG іконки залишається без змін) ... */}
        <defs>
...
export const UserProfile = ({user, onUserUpdate}) => {
    ...
    const [isModalOpen, setIsModalOpen] = useState(false); // Інвентар
    const [isVipPromoOpen, setIsVipPromoOpen] = useState(false); // ✨ VIP промо

    // --- useEffect для завантаження персонажів (без змін) ---
    useEffect(() => {
    ...
    // --- Функція для зміни персонажа (без змін) ---
    ...
    // --- Логіка завантаження/помилок (без змін) ---
    ...
    /** Обробник кліку на весь профіль (ВИДАЛЕНО) */
    // const handleProfileClick = () => { ... };

    // --- Обробник покупки (без змін) ---

    // --- ✨ Нові обробники кліків ---
    ...
    return (
        <>
            ...
            {/* ✨ Оновлений профіль з умовним onClick та title */}
            <div
                className={styles.userProfile}
                title={showVipPromo ? "Отримати VIP-статус" : "Відкрити інвентар"}
            >
                <div className={styles.characterSwitcher}>
                    <AnimatePresence>
                        {/* ✨ Умова 1: Є кілька персонажів */}
                        {hasMultipleCharacters && nextCharacter && (
                            <motion.img
                                ...
                                alt="Next character"
                                onClick={(e) => e.stopPropagation()} // Клік на другого персонажа нічого не робить
                        ...
                        {showVipPromo && (
                            <motion.img
                                ...
                                alt="Купити VIP"
                                onClick={handleVipPromoOpen} // ✨ Клік на силует = VIP промо
                                title="Отримати VIP-статус"   // ✨ Додамо title
                        ...
                        <motion.img
                            ...
                            onClick={handleInventoryOpen} // ✨ Клік на персонажа = Інвентар
                            title="Відкрити інвентар"     // ✨ Додамо title
                            style={{cursor: 'pointer'}}   // ✨ Покажемо, що можна клікнути
                    ...
                    {/* ✨ Кнопка перемикання (показується або для VIP, або для промо) */}
                    {(hasMultipleCharacters || showVipPromo) && (
                        <button
                            // ✨ Додаємо клас для "затемнення", якщо це промо
                            className={`${styles.switchButton} ${showVipPromo ? styles.switchButtonPromo : ''}`}
                            onClick={handleSwitchClick} // ✨ Новий обробник
                            disabled={isSwitching}
                            title={showVipPromo ? "Отримати VIP" : "Змінити персонажа"} // ✨ Умовний title
                        >
                            {isSwitching ? '...' : (
                                <svg width="48" height="48" viewBox="0 0 48 48" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    {/* ... (код SVG кнопки без змін) ... */}
                                    <defs>
                ...
                {/* ✨ Оновлений Інфо-бокс: onClick перенесено на дочірні елементи */}
                <div className={styles.infoBox}>
                    <div
                        className={styles.leftSection}
                        onClick={handleInventoryOpen} // ✨ Клік на ліву частину = Інвентар
                    ...
                    >
                        ...
                        <div className={styles.locationGroup}>
                            <img className={styles.locationIcon} src={Config.IMAGES.country}
                                 alt={`${currentCharacter.country} flag`}/>
                            <span className={styles.location}>{currentCharacter.country}</span>
                        </div>
                    </div>
                    <div
                        className={styles.stats}
                        onClick={handleInventoryOpen} // ✨ Клік на праву частину = Інвентар
                        title="Відкрити інвентар"
                        style={{cursor: 'pointer'}}
                    >
