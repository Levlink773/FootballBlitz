...
class RemniderCharacterService:
    ...
    @classmethod
    async def create_character_reminder(cls, character_id: int) -> ReminderCharacter | None:
    ...
    @classmethod
    async def get_characters_in_training(cls) -> list[ReminderCharacter] | None:
        async for session in get_session():
            async with session.begin():
                ...
                return list(characters_in_training)
    ...
    @classmethod
    async def toggle_character_training_status(cls, character_id: int) -> ReminderCharacter | None:
        async for session in get_session():
            async with session.begin():
    ...
    async def get_characters_not_training(cls) -> list[ReminderCharacter] | None:
        async for session in get_session():
            async with session.begin():
                ...
                return list(characters_not_training)
            
        
    @classmethod
    async def character_in_training(cls, character_id: int) -> Any | None:
        async for session in get_session():
            async with session.begin():
                result = await session.execute(
                    select(ReminderCharacter)
